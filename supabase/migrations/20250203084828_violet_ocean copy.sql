...
DECLARE
  city text;
BEGIN
  -- Extract city from "Location" field format (e.g. "Bangor, ME")
...
COMMENT ON FUNCTION extract_city_from_location IS 'Extracts city name from the Location field (e.g. "Bangor, ME" -> "Bangor")';
COMMENT ON FUNCTION ensure_city_location_exists IS 'Ensures a location exists for a given city from the Location field, creating it if necessary';