...
-- Create helper function to get folder path
CREATE OR REPLACE FUNCTION storage.get_folder_path(path text)
RETURNS text AS $$
BEGIN
  RETURN REGEXP_REPLACE(path, '^([^/]+/[^/]+)/.*$', '\1');
END;
...
USING (
  ...
  storage.get_folder_path(name) = 'post-featured/iyc'
);
