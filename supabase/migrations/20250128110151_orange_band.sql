...
-- Add category associations for sponsorships
WITH sponsorship_posts AS (
...
SELECT sponsorship_posts.id, categories.id
FROM sponsorship_posts
CROSS JOIN categories
WHERE categories.slug = 'sponsorship';

-- Add category associations for donations
WITH donation_posts AS (
  SELECT id FROM posts WHERE slug IN (
    'emergency-food-pantry-support',
    'school-supplies-success-program',
    'winter-warmth-fund',
...
INSERT INTO post_categories (post_id, category_id)
SELECT donation_posts.id, categories.id
FROM donation_posts
CROSS JOIN categories
WHERE categories.slug = 'donation';