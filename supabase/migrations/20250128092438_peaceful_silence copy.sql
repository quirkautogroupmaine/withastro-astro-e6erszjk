...
WHERE 
  (p.slug = 'all-new-2024-chevrolet-silverado-ev-arrives' AND c.slug IN ('news', 'vehicle-launch'))
  OR (p.slug = 'quirk-subaru-new-facility-announcement' AND c.slug IN ('news', 'company-update'))
  OR (p.slug = 'sarah-johnson-promotion-announcement' AND c.slug IN ('news', 'employee-news'))
  OR (p.slug = 'mobile-maintenance-service-launch' AND c.slug IN ('news', 'company-update'))
  OR (p.slug = 'new-mobile-app-launch' AND c.slug IN ('news', 'company-update'));